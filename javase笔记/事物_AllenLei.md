...
## 并发访问存在的问题
- 脏读
- 不可重复度：一个事务多次读取同一条记录得到的结果不一样，一般是由update操作引起
- 幻读：一个事务前后查询数据的记录数不一致，一般是由于insert/delete操作引起。

## 事务的隔离级别
- read uncommitted;读未提交，存在三个问题。
- read committed;读已提交，存在不可重复读，幻读
- repeatable read；可重复读，存在幻读问题
- serializable；串行化，不存在问题

## ThreadLocal类
- 作用：提供线程内的局部变量，该变量可以在线程的生命周期中有效，可以在线程的多个方法中共享变量。
- 结构：底层是一个map集合，键： 当前线程对象 值：共享值
